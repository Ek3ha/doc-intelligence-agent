...
# ══════════════════════════════════════════════════════════════════════════════
st.title("🧠 Autonomous Document Intelligence Agent")
st.caption("A demo of Retrieval-Augmented Generation (RAG) using LangChain + FAISS + Groq llama3")
...
with col2:
    st.markdown("#### 📊 CSV")
    st.markdown("- Sales data queries\n- Survey result analysis\n- Inventory insights\n- Employee record lookup")
with col3:
    st.markdown("#### 📝 TXT")
    st.markdown("- Log file investigation\n- Policy document search\n- Meeting notes Q&A\n- Knowledge base queries")
...
if st.session_state.file_loaded:
    ...
    for msg in st.session_state.messages:
        with st.chat_message(msg["role"]):
            st.markdown(msg["content"])
            # show source chunks if they exist
            if msg["role"] == "assistant" and msg.get("chunks"):
                with st.expander(f"📚 Source chunks used ({len(msg['chunks'])})"):
                    for i, chunk in enumerate(msg["chunks"]):
                        st.markdown(f"**Chunk {i+1}**")
                        st.caption(f"Page: {chunk.metadata.get('page', 'N/A')} | Source: {chunk.metadata.get('source', 'N/A')}")
    ...
    if prompt := st.chat_input("Type your question…"):
        ...
        with st.chat_message("assistant"):
            ...
            if chunks:
                with st.expander(f"📚 Source chunks used ({len(chunks)})"):
